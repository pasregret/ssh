...
<BODY>
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：客户管理 &gt; 客户分页列表</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
	...
	<TR
		...
		<TD>${customer.dictCusLevel.dname}</TD>
...
<c:if test="${pageBean.currentPage!=1}">
	[<A href="${pageContext.request.contextPath }/customer_findPage.action?currentPage=${pageBean.currentPage-1}">前一页</A>]
</c:if>


<c:if test="${pageBean.currentPage!=pageBean.totalPage}">
	[<A href="${pageContext.request.contextPath }/customer_findPage.action?currentPage=${pageBean.currentPage+1}">后一页</A>] 
